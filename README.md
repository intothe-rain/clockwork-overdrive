...
1. **실행**: `index.html` 파일을 웹 브라우저(Chrome, Edge, Firefox, Whale 등)로 열어 실행합니다.
2. **조작법**:
   - **타이밍 타격**: `Space` 키 또는 `마우스 좌클릭`
   - **일시정지 / 재개**: `ESC` 키
3. **목표**:
...
### 1. 🏆 명예의 전당 (Hall of Fame Top 10 랭킹)
- 게임 오버 시 10위권 진입 시 **콜사인(Callsign) 닉네임 입력 모달** 팝업 및 영구 랭킹 등록!
- 순위, 플레이어명, 점수, 최대 콤보, 최고 BPM, 달성 일자 기록.
- 1위(금), 2위(은), 3위(동) 메달 뱃지 및 스팀펑크 레전드 랭커(Tesla, Lovelace 등)와의 경쟁!
